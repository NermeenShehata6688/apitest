    ...
    internal class UserService : IUserService
    {
        ...
        public ResponseDto GetUsers(UserSearchDto userSearchDto)
        {
            try
            {
                ...
                var mapper = new PaginateDto<VwUserDto> { Count = lstToSend.Count(), Items = lstToSend != null ? lstUserDto.Skip(userSearchDto.Index == null || userSearchDto.PageSize == null ? 0 : ((userSearchDto.Index.Value - 1) * userSearchDto.PageSize.Value)).Take(userSearchDto.PageSize ??= 1000).ToList() : lstToSend.ToList() };
