...
namespace IesSchool.Core.Services
{
    internal class MobileService : IMobileService
    {
        ...
        public ResponseDto GetEvents(GetMobileEventsDto? getMobileEventsDto)
        {
            try
            {
                ...
                if (getMobileEventsDto.Id != null)
                {
                    ...
                    var events = _uow.GetRepository<Event>().GetList(x => x.IsDeleted != true && x.IsPublished == true && studentEventIds.Contains(x.Id), x => x.OrderByDescending(x => x.FromDate), x => x
        ...
        public ResponseDto GetStudentProgressReports(int studentId)
        {
            try
            {
                var iepProgressReport = _uow.GetRepository<IepProgressReport>().GetList(x => x.StudentId == studentId && x.IsDeleted != true, null, x => x
                   .Include(x => x.AcadmicYear)
                   .Include(x => x.Iep)
                   .Include(s => s.Teacher).Include(x => x.Term), 0, 100000, true);
                var iepMapper = _mapper.Map<PaginateDto<IepProgressReportDto>>(iepProgressReport);
                if (iepMapper.Items.Count() > 0)
                {
                    iepMapper.Items = iepMapper.Items.OrderByDescending(x => x.Date).Where(x=> x.IepStatus==3 && x.IsIepPublished==true).ToList();
                    return new ResponseDto { Status = 1, Message = " Seccess", Data = iepMapper };
                }
                else
                    return new ResponseDto { Status = 1, Message = " No Data", Data = iepMapper };
        ...
        public FileStreamResult IepReportPDF(int iepId)
        {
            try
            {
                using (ExcelEngine excelEngine = new ExcelEngine())
                {
                    ...
                    FileStreamResult fileStreamResult = new FileStreamResult(stream, "application/pdf");
                    fileStreamResult.FileDownloadName = (studentName == null ? "" : studentName + "-IEPReport" + ".pdf");
        ...
        public FileStreamResult IepProgressReportPDF(int iepProgressReportId)
        {
            try
            {
                using (ExcelEngine excelEngine = new ExcelEngine())
                {
                    ...
                    FileStreamResult fileStreamResult = new FileStreamResult(stream, "application/pdf");
                    fileStreamResult.FileDownloadName = ("ProgressReport" + ".pdf");
        ...
        public FileStreamResult ItpReportPDF(int itpId)
        {
            try
            {
                using (ExcelEngine excelEngine = new ExcelEngine())
                {
                    ...
                    FileStreamResult fileStreamResult = new FileStreamResult(stream, "application/pdf");
                    fileStreamResult.FileDownloadName = ("ItpReport" + ".pdf");
        ...
        public FileStreamResult ItpProgressReportPDF(int itpProgressReportId)
        {
            try
            {
                using (ExcelEngine excelEngine = new ExcelEngine())
                {
                    ...
                    FileStreamResult fileStreamResult = new FileStreamResult(stream, "application/pdf");
                    fileStreamResult.FileDownloadName = ("ItpProgressReport" + ".pdf");
        ...
        public FileStreamResult IxpReportPDF(int ixpId)
        {
            try
            {
                using (ExcelEngine excelEngine = new ExcelEngine())
                {
                    ...
                    FileStreamResult fileStreamResult = new FileStreamResult(stream, "application/pdf");
                    fileStreamResult.FileDownloadName = ("IxpReport" + ".pdf");
