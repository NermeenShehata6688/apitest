...
namespace IesSchool.Core.Services
{
    internal class EventService : IEventService
    {
        ...
        public ResponseDto IsPublished(EventDto oEventDto)
        {
            try
            {
                Event oEvent = _uow.GetRepository<Event>().Single(x => x.Id == oEventDto.Id);
                oEvent.IsPublished = oEventDto.IsPublished;
                _uow.GetRepository<Event>().Update(oEvent);
                _uow.SaveChanges();
