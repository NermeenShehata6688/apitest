...
namespace IesSchool.Core.Services
{
    internal class EventService : IEventService
    {
        ...
        public ResponseDto GetEvents()
        {
            try
            {
                var allEvents = _uow.GetRepository<Event>().GetList(x => x.IsDeleted != true, x=>x.OrderByDescending(x=>x.FromDate), x=> x.Include(x=>x.Department).Include(x => x.EventType), 0, 100000, true);
